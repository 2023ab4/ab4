...
# ╔═╡ 5a3c4a14-54f8-443a-bf64-107ef31f167f
import Flux
...
_models_root_dir = "../data_indep"

# ╔═╡ f78aaf59-44c1-4e9e-a8e2-a59807862fd2
model_black_blue, states_black_blue = JLD2.load("$_models_root_dir/indep_black+blue.jld2", "model", "states");

# ╔═╡ 271399f5-9890-4541-9379-443c91751c57
model_blue_both, states_blue_both = JLD2.load("$_models_root_dir/indep_blue+both.jld2", "model", "states");

# ╔═╡ 74c8518e-e69d-407e-96d7-d7c20329960a
model_black_both, states_black_both = JLD2.load("$_models_root_dir/indep_black+both.jld2", "model", "states");

# ╔═╡ 1a43945a-b24c-4e71-b759-38d3239a7beb
model_blue, states_blue = JLD2.load("$_models_root_dir/indep_blue.jld2.2", "model", "states");

# ╔═╡ d22033df-eb5f-462d-b7ca-cbcf43dec49a
model_both, states_both = JLD2.load("$_models_root_dir/indep_both.jld2.2", "model", "states");
...
Makie.scatter(
	vec(Ab4Paper2023.init_deep_from_indep(model_both.states[1]; ϵ=0.1, b0=20).m(_y)),
	vec(model_both.states[1].m(_y))
)
...
let fig = Makie.Figure()
	...
	Makie.lines!(ax, get(JLD2.load("$_models_root_dir/indep_black+blue.jld2", "history")[:loglikelihood])...)
	ax = Makie.Axis(fig[1,2]; width, height, title="Blue+Both")
	Makie.lines!(ax, get(JLD2.load("$_models_root_dir/indep_blue+both.jld2", "history")[:loglikelihood])...)
	ax = Makie.Axis(fig[1,3]; width, height, title="Blue")
	Makie.lines!(ax, get(JLD2.load("$_models_root_dir/indep_blue.jld2.2", "history")[:loglikelihood])...)
	ax = Makie.Axis(fig[1,4]; width, height, title="Both")
	Makie.lines!(ax, get(JLD2.load("$_models_root_dir/indep_both.jld2.2", "history")[:loglikelihood])...)
	# ax = Makie.Axis(fig[1,5]; width, height, title="Black+Blue+Both")
	# Makie.lines!(ax, get(JLD2.load("$_models_root_dir/indep_black+blue+both.jld2", "history")[:loglikelihood])...)
...
begin
	model_black_blue_swap, states_black_blue_swap = JLD2.load("$_models_root_dir/indep_black+blue.jld2", "model", "states");
...
begin
	model_blue_both_swap, states_blue_both_swap = JLD2.load("$_models_root_dir/indep_blue+both.jld2", "model", "states");
...
begin
	...
	model_blue_only_swap, states_blue_only_swap = JLD2.load("$_models_root_dir/indep_blue.jld2.2", "model", "states");
...
begin
	model_both_swap, states_both_swap = JLD2.load("$_models_root_dir/indep_both.jld2.2", "model", "states")
